...
type WorkTag = 'FunctionComponent' | 'HostComponent' | 'HostText' | 'Fragment' | 'ContextProvider' | 'ContextConsumer' | 'Memo'
...
function myCreateFiber(element: MyElement | string, returnFiber: MyFiber | null): MyFiber {
  ...
  const tag: MyFiber['tag'] =
    element.type === 'function' ? 'FunctionComponent' : 'HostComponent'
  ...
  return (
    <div style={{ marginLeft: depth * 20 }}>
      ...
      >
        ...
        {fiber.tag === 'HostText' && (
          <span style={{ color: '#6b7280' }}> &quot;{String(fiber.memoizedProps.text)}&quot;</span>
        )}
      </div>
...
function ElementTree({ el, depth = 0 }: { el: MyElement | string; depth?: number }) {
  ...
  return (
    <div style={{ marginLeft: depth * 20 }}>
      ...
      >
        &lt;
        <span style={{ color: el.type === 'function' ? '#3b82f6' : '#10b981' }}>
          {el.typeName}
        </span>
        &gt;
      </div>
      {el.children.map((c, i) => (
        <ElementTree key={i} el={c} depth={depth + 1} />
      ))}
    </div>
  )
}
    ...
    <div className="exercise-container">
      ...
      <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: '180px' }}>
            ...
            {selectedFiber ? (
              <>
                <div style={{ color: '#9ca3af', marginBottom: '8px' }}>
                  {getFiberName(selectedFiber)}
                </div>
                {(
                  [
                    ['type', selectedFiber.type ?? 'null'],
                    ['tag', selectedFiber.tag],
                    ['child', getFiberName(selectedFiber.child)],
                    ['sibling', getFiberName(selectedFiber.sibling)],
                    ['return', getFiberName(selectedFiber.returnFiber)],
                  ] as [string, string][]
                ).map(([k, v]) => (
                  <div key={k} style={{ color: '#6b7280', marginBottom: '2px' }}>
                    {k}: <span style={{ color: '#e5e7eb' }}>{v}</span>
                  </div>
                ))}
              </>
            ) : (
          ...
          <div style={{ marginTop: '12px', fontSize: '12px', color: '#6b7280', marginBottom: '4px' }}>
            Узлы (кликабельны):
          </div>
          {allFibers.map((f, i) => (
            <button
              ...
              style={{
                ...
                borderRadius: '4px',
                background: selectedFiber === f ? '#1f2937' : '#111827',
                color: '#9ca3af',
                cursor: 'pointer',
                fontSize: '12px',
                fontFamily: 'monospace',
...
function flattenToDom(fiber: FiberNodeDef3): DomNode[] {
  if (HOST_TAGS.includes(fiber.tag)) {
    const domChildren = (fiber.children ?? []).flatMap(c => flattenToDom(c))
    return [{ name: fiber.name, children: domChildren }]
  }
  // Transparent: FC, Fragment, ContextProvider, ContextConsumer, Memo
    ...
    <div style={{ marginLeft: depth * 16 }}>
      <div
        style={{
          ...
          padding: '3px 8px',
          borderRadius: '4px',
          background: isHost ? '#0f2a1e' : '#1a1a2e',
          border: `1px solid ${color}33`,
          fontSize: '12px',
          marginBottom: '4px',
          opacity: isHost ? 1 : 0.65,
          fontStyle: isHost ? 'normal' : 'italic',
        }}
      >
      ...
      {(node.children ?? []).map((c, i) => (
        <FiberTreeView key={i} node={c} depth={depth + 1} />
      ))}
    </div>
  )
}
...
function DomTreeView({ node, depth = 0 }: { node: DomNode; depth?: number }) {
  return (
    <div style={{ marginLeft: depth * 16 }}>
      ...
      {node.children.map((c, i) => (
        <DomTreeView key={i} node={c} depth={depth + 1} />
      ))}
    </div>
  )
}

const EXAMPLES: { label: string; tree: FiberNodeDef3 }[] = [
  {
    ...
    <div className="exercise-container">
        ...
        {EXAMPLES.map((ex, i) => (
          <button
            ...
            style={{
              padding: '6px 14px',
              borderRadius: '6px',
              border: exampleIdx === i ? '2px solid #3b82f6' : '2px solid transparent',
              background: exampleIdx === i ? '#1e3a5f' : '#1f2937',
              color: '#e5e7eb',
              cursor: 'pointer',
              fontSize: '13px',
            }}
      ...
      <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: '220px' }}>
          <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '8px' }}>
            Fiber-дерево (все узлы)
          </div>
          <div
            style={{ background: '#0d1117', padding: '12px', borderRadius: '6px', minHeight: '120px' }}
          >
            <FiberTreeView node={example.tree} />
          </div>
          <div style={{ marginTop: '8px', fontSize: '11px', color: '#4b5563', fontStyle: 'italic' }}>
            Полупрозрачные узлы не создают DOM
          </div>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', fontSize: '24px', color: '#374151' }}>
          →
        </div>

        <div style={{ flex: 1, minWidth: '220px' }}>
          <div style={{ fontSize: '12px', color: '#6b7280', marginBottom: '8px' }}>
            DOM-дерево (только host-узлы)
          </div>
          <div
            style={{ background: '#0d1117', padding: '12px', borderRadius: '6px', minHeight: '120px' }}
          >
            {domTree.map((n, i) => (
              <DomTreeView key={i} node={n} />
            ))}
          </div>
          <div style={{ marginTop: '8px', fontSize: '11px', color: '#4b5563', fontStyle: 'italic' }}>
            Fragment, FC, Provider исчезли
          </div>
        </div>
      </div>
      ...
      >
        <strong style={{ color: '#e5e7eb' }}>flattenToDom:</strong> обходит Fiber-дерево рекурсивно.
        HostComponent и HostText — остаются. Fragment, FunctionComponent, ContextProvider, Memo —
        прозрачны: их дети &quot;всплывают&quot; к ближайшему HostComponent-предку.
      </div>
    </div>
  )
}
...
function traverseFiber(root: Fiber4): FiberDiagnostic[] {
  ...
  while (workInProgress !== null) {
    ...
    result.push({
      ...
      depth,
      childCount,
      hasChildren: workInProgress.child !== null,
      hasSibling: workInProgress.sibling !== null,
      returnName: workInProgress.returnFiber?.name ?? null,
    })
...
type FilterTag = 'all' | FiberTag4

const TAG_BADGE_BG: Record<FiberTag4, string> = {
  FunctionComponent: '#1e3a5f',
  HostComponent: '#0f2a1e',
  HostText: '#2d1b00',
  Fragment: '#1f2937',
  ContextProvider: '#2d1b4e',
}

const TAG_BADGE_TEXT: Record<FiberTag4, string> = {
  FunctionComponent: '#60a5fa',
  HostComponent: '#34d399',
  HostText: '#fbbf24',
  Fragment: '#9ca3af',
  ContextProvider: '#a78bfa',
}

export function Task1_4_Solution() {
  const [filter, setFilter] = useState<FilterTag>('all')
  const fiberRoot = buildFiber4(TREE_4, null)
  const diagnostics = traverseFiber(fiberRoot)

  const filtered =
    filter === 'all' ? diagnostics : diagnostics.filter(d => d.tag === filter)
  ...
  return (
    <div className="exercise-container">
      ...
      <div style={{ display: 'flex', gap: '16px', marginBottom: '16px', flexWrap: 'wrap' }}>
        {(
          [
            { label: 'Всего узлов', value: diagnostics.length, color: '#e5e7eb' },
            { label: 'Макс. глубина', value: maxDepth, color: '#60a5fa' },
            { label: 'HostComponent', value: hostCount, color: '#34d399' },
      ...
      <div style={{ overflowX: 'auto' }}>
        <table
          style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px', fontFamily: 'monospace' }}
        >
          <thead>
          ...
          <tbody>
            {filtered.map((d, i) => (
              <tr
                key={i}
                style={{
                  background: TAG_BADGE_BG[d.tag] ?? '#111827',
                  borderBottom: '1px solid #1f2937',
                }}
              >
                <td style={{ padding: '6px 12px', color: '#e5e7eb', fontFamily: 'monospace' }}>
                  {'\u00a0\u00a0'.repeat(d.depth)}
                  {d.name}
                </td>
                <td style={{ padding: '6px 12px' }}>
                  <span
                    style={{
                      padding: '2px 6px',
                      borderRadius: '3px',
                      background: TAG_BADGE_BG[d.tag],
                      color: TAG_BADGE_TEXT[d.tag],
                      fontSize: '11px',
                      border: `1px solid ${TAG_BADGE_TEXT[d.tag]}44`,
                    }}
                  >
                    {d.tag}
                  </span>
                ...
                <td style={{ padding: '6px 12px', color: d.childCount > 0 ? '#e5e7eb' : '#374151' }}>
                  {d.childCount}
                </td>
                <td style={{ padding: '6px 12px', color: d.hasSibling ? '#34d399' : '#374151' }}>
                  {d.hasSibling ? '✓' : '—'}
                </td>
