  ...
  return (
    <div className="exercise-container">
          ...
          {hooks.map((hook, index) => (
            <div key={hook.name} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
              ...
              >
                ...
                <div style={{ color: '#e5e7eb', marginBottom: '8px', fontWeight: 600 }}>
                  {hook.memoizedState}
                </div>
      ...
      >
        ...
        >
{`// Mount (isLoggedIn = true): 3 узла создаются
...
// Simulating fiber.memoizedState as a global array
...
function myUseState<T>(initialValue: T): [T, (newValue: T) => void] {
  ...
  // If this slot hasn't been initialized — mount phase
...
export function Task4_2_Solution() {
  ...
  // Reset cursor before each render — simulating React's behavior
  cursor = 0
  ...
  return (
    <div className="exercise-container">
      ...
      >
        ...
        <div>
          cursor после рендера:{' '}
          <span style={{ color: '#f59e0b' }}>{cursor}</span>
          <span style={{ color: '#4b5563' }}> (2 хука = 2 слота)</span>
        </div>
        <div style={{ marginTop: '8px', color: '#4b5563', fontSize: '12px' }}>
          // cursor = 0 в начале каждого рендера
          <br />
          // каждый myUseState(x) берёт cursor++ слот
        </div>
      </div>
    </div>
  )
...
function TimerColumn({ title, description, accentColor, variant }: TimerColumnProps) {
  const [count, setCount] = useState(0)
  ...
  useEffect(() => {
    ...
    if (variant === 'bug') {
      ...
      id = setInterval(() => {
        setCount(count + 1) // count is stale — always 0 from closure
      }, 1000)
    } else if (variant === 'functional') {
      id = setInterval(() => {
        setCount(prev => prev + 1) // React provides current value
      }, 1000)
    } else {
      id = setInterval(() => {
        setCount(countRef.current + 1) // ref is always up to date
      }, 1000)
    }

    return () => clearInterval(id)
  // Intentionally only depend on `running` for the bug demo
  }, [running]) // eslint-disable-line react-hooks/exhaustive-deps

  const isBug = variant === 'bug'
  const borderColor = isBug ? '#7f1d1d' : '#14532d'
  const bg = isBug ? '#1a0f0f' : '#0f1a0f'
  const numColor = isBug ? '#f87171' : '#4ade80'
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        <button
          ...
          style={{
            ...
            borderRadius: '6px',
            border: 'none',
            background: running ? '#374151' : accentColor,
            color: '#fff',
            cursor: 'pointer',
            fontSize: '13px',
            fontWeight: 600,
    ...
    <div className="exercise-container">
      ...
      <div style={{ display: 'flex', gap: '16px', marginBottom: '24px', flexWrap: 'wrap' }}>
        <TimerColumn
          ...
          variant="bug"
          description={
            'setCount(count + 1) — count захвачен при запуске эффекта. ' +
            'Всегда равен 0, поэтому счётчик застревает на 1.'
          }
        />
        <TimerColumn
          ...
          variant="functional"
          description={
            'setCount(prev => prev + 1) — React сам передаёт актуальный prev ' +
            'из очереди обновлений. Замыкание не нужно.'
          }
        />
        <TimerColumn
          ...
          description={
            ...
            'setInterval читает ref — всегда актуальное значение.'
          }
        />
      </div>
      ...
      >
        <strong style={{ color: '#e5e7eb' }}>Почему баг происходит:</strong>
        <br />
        Каждый рендер создаёт новое замыкание с новым значением count. Но useEffect с{' '}
        <code style={{ color: '#f59e0b' }}>[]</code> запускается один раз — его замыкание
        захватывает count = 0. setInterval живёт в этом старом замыкании и всегда видит 0.
        <br />
        <br />
        <strong style={{ color: '#e5e7eb' }}>Запусти все три таймера</strong> и посмотри разницу через 5 секунд.
      </div>
    </div>
  )
}
...
export function Task4_4_Solution() {
  ...
  return (
    <div className="exercise-container">
      ...
      >
        <strong style={{ color: '#e5e7eb' }}>Почему плохой вариант даёт 2 рендера:</strong>
        <br />
        1. Изменился firstName → рендер #1 (fullName ещё старый из прошлого state)
        <br />
        2. useEffect видит изменение deps → вызывает setFullName → рендер #2
        <br />
        <br />
        <strong style={{ color: '#e5e7eb' }}>Хороший вариант:</strong> fullName вычисляется в
        рендере #1 и сразу правильный. Никакого useEffect, никакого лишнего state.
        <br />
        <br />
        <span style={{ color: '#6b7280' }}>
          Правило: если значение полностью определяется props или другим state —
          вычисляй его во время рендера. Это derived value, а не самостоятельное состояние.
        </span>
      </div>
    </div>
  )
