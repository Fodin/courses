...
flowchart TD
    ...
    App --> HTTP["http / https modules\nHTTP parsing, routing, headers"]
    HTTP --> TLS["tls module (optional)\nTLS/SSL encryption layer"]
    TLS --> Net["net module\nTCP sockets, IPC"]
...
sequenceDiagram
    participant Client
    participant Server
    Client->>Server: ClientHello (versions, ciphers)
    Server->>Client: ServerHello (chosen version, cipher)
    Server->>Client: Certificate (X.509 cert)
    Note over Client: Verify cert chain
