...
block-beta
    columns 1
    layer4["Слой 4: COPY app.js (R/O) -- самый верхний"]
    layer3["Слой 3: apt install curl (R/O)"]
    layer2["Слой 2: apt-get update (R/O)"]
    layer1["Слой 1: ubuntu:22.04 (R/O) -- базовый"]
```
...
block-beta
    columns 1
    writable["Записываемый слой контейнера (R/W) -- только здесь можно менять файлы"]
    layer4["Слой 4: COPY app.js (R/O)"]
    layer3["Слой 3: apt install curl (R/O)"]
    layer2["Слой 2: apt-get update (R/O)"]
    layer1["Слой 1: ubuntu:22.04 (R/O)"]
```
