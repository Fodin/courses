...
export function Task0_2_Solution() {
  ...
  // Every call of this function = one Render
  renderCount.current++

  // Every commit triggers this effect
  useEffect(() => {
  ...
  return (
    <div className="exercise-container">
      ...
      >
        ...
        <br />
        "Force re-render" всегда увеличивает оба счётчика — Render всегда приводит к Commit, если результат изменился.
        <br />
        "Same value setState" — Render может случиться (React вызывает функцию компонента), но если
        результат тот же — Commit не происходит. Render ≠ DOM update.
        <br />
        <em style={{ color: '#6b7280' }}>В Strict Mode renderCount растёт быстрее — React вызывает функции дважды для проверки чистоты.</em>
      </div>

      {/* Hidden: just to use trigger in JSX so it doesn't get optimized away */}
...
export function Task0_3_Solution() {
  ...
  const handleRender: ProfilerOnRenderCallback = (
    ...
    // Force table to re-render with new data
    forceUpdate(n => n + 1)
  }
